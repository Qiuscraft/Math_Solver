...
model = PeftModel.from_pretrained(model, model_id="./output/Qwen/checkpoint-2895/")

with open("submit.csv", 'w', encoding='utf-8', newline='') as file:
    ...
    for idx, row in tqdm(enumerate(test_data)):
        ...
        csv_writer.writerow([id, response.replace('\n', '\\n')])
        print("\n==============================")
        print(f"system_prompt: {instruction}")
        print(f"user_prompt: {input_value}")
